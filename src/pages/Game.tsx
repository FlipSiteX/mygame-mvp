...
const Game = () => {
	...
	const addPointUser = (activeUser: IUser | null, points: number) => {
		socket.emit("addPoints", { activeUser: activeUser, points: points });
		hiddenQuestion();
	};
			...
			<div className='w-full h-screen flex-col justify-center items-center'>
				...
				<div className=' bg-white rounded-lg p-4 flex flex-col gap-y-3 items-center'>
					...
					{selectedQuestion?.points && (
						<h2 className='text-wrap font-bold text-xl text-center'>
					...
					{selectedQuestion?.desc && (
						<p
							...
							className={classNames(
								showDesc
									? "text-center text-xl"
									: "text-[#007953] underline text-center text-xl"
							)}
						>
							{showDesc ? selectedQuestion?.desc : "Показать описание"}
						</p>
					)}
					{showDesc && (
						<p
							onClick={() => setShowDesc(false)}
							className='text-[#007953] underline text-center text-xl'
						>
							Скрыть описание
						</p>
					)}
					...
					{selectedQuestion &&
						...
						!queue.find((el: IUser) => el.username == user?.username) && (
							<button
								...
								className='w-40 h-40 mt-2 rounded-full text-2xl bg-green-300'
							>
								Ответить
							</button>
						)}
					{selectedQuestion &&
						isSkipped &&
						!queue.find((el: IUser) => el.username == user?.username) && (
							<p className='text-xl'>Вопрос пропущен</p>
						)}
					{selectedQuestion &&
						queue.find((el: IUser) => el.username == user?.username) && (
							<p className='text-xl'>
	...
	return (
		<div className='w-full min-h-screen flex flex-col relative bg-background-img bg-cover'>
			{isSelect && (
				<div className='flex w-full justify-center mt-8'>
					<div className='w-[60%] 2xl:w-[1200px] max-h-[920px] flex flex-col p-4 gap-y-6 rounded-lg bg-white'>
						<div className='space-y-4'>
							<h2 className='text-center text-2xl font-bold'>
								Вопрос за {selectedQuestion?.points}
							</h2>
							{selectedQuestion?.desc && (
								<p className='text-center text-2xl hidden 2xl:block'>
									{selectedQuestion?.desc}
								</p>
							)}
							{selectedQuestion?.desc && (
						...
						{selectedQuestion?.question_type == "img" && (
							<div className='flex justify-center rounded-lg overflow-hidden'>
								<img
									src={`http://192.168.1.33:8000/${selectedQuestion.question_file}`}
									alt=''
									className='rounded-lg w-full 2xl:max-h-[700px] max-h-[500px] object-contain'
								/>
							</div>
						)}
					...
					<div className='2xl:w-[20%] w-[30%] h-max ml-12 flex flex-col text-xl p-4 rounded-lg gap-y-6 bg-white'>
						{queue.length > 0 ? (
							[...queue]?.map((user) => (
								<div
									key={user.username}
								...
								>
									<h2 className='mr-4'>{user.username}</h2>
									<h2>Очки: {user.points}</h2>
								</div>
							))
						) : (
							<button
								className='w-full p-2 bg-red-300 text-xl rounded-lg'
								onClick={() => {
									setCompletedQuestionsCount(completedQuestionsCount + 1);
									addPointUser(null, 0);
								}}
							...
							<div className='flex flex-col text-xl gap-y-3'>
								<button
									...
									onClick={() => {
										...
										addPointUser(
											activeUser as IUser,
											(selectedQuestion as IQuestion)?.points
										);
									}}
								>
				...
				<div className='flex w-full justify-center mt-8'>
					<div className='w-[60%] 2xl:w-[1200px] max-h-[920px] flex flex-col p-4 gap-y-6 rounded-lg bg-white'>
						...
						{selectedQuestion?.answer_type == "img" && (
							<div className='flex justify-center rounded-lg overflow-hidden'>
								<img
									src={`http://192.168.1.33:8000/${selectedQuestion.answer_file}`}
									alt=''
									className='rounded-lg w-full 2xl:max-h-[700px] max-h-[500px] object-contain'
								/>
							</div>
					...
					<div className='2xl:w-[20%] w-[30%] h-max ml-12 p-4 rounded-lg bg-white'>
						<button
							className='w-full p-2 bg-red-300 text-xl rounded-lg'
			...
			{!isSelect && !isAnswer && (
				<div className='grid grid-cols-4 justify-between items-center my-4 p-4'>
					...
					<div className='col-end-5 justify-self-end mr-[30px]'>
						<button
							className='text-4xl font-bold text-center underline decoration-2'
							onClick={() => {
								setIsUser(!isUser);
							}}
						>
			...
			{!isSelect &&
				...
				(!isUser && completedQuestionsCount < 25 ? (
									...
									{topic.questions.map((question: IQuestion) => {
										if (!question.isHidden) {
											return (
												...
												>
													<h2 className='text-center text-5xl font-bold leading-none'>
														{question.points}
													</h2>
												</div>
											);
					...
					<div className='flex w-[800px] bg-white p-4 rounded-lg mx-auto flex-col gap-y-3'>
						...
						{completedQuestionsCount > 25 && (
							<h2 className='text-2xl font-bold'>Итоги</h2>
						)}
						{!isPointsCorrect && (
							<h2 className='text-2xl'>
								Кому назначить очки ({lastQuestion?.points}) за последний
								вопрос:
							</h2>
						)}
